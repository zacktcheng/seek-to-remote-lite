...
public class RemotiveClient {
	...
	// For recommendItems() purpose, set it false to store job descriptions in Item instances, and for vast search.
	...
	public List<Item> search(String category) {
		...
		try {
			...
			// Return the jobs in the response body in when the response code is 200 and the response has contents.
	...
	private static Item getItemForItemList(JSONObject object) {
		...
		Item item = new Item.Builder()
				.itemId(Helper.getStringFieldOrEmpty(object, "id"))
	...
	// Check out https://remotive.io/api/remote-jobs/categories for predefined categories.
